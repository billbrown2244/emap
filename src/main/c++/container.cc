...
EmapContainer::EmapContainer(fluid_synth_t* synth_new) {
	...
	bool file_exists = json_parser_load_from_file(parser, config_file.c_str(),
			NULL);
	...
	if (file_exists && (nodepath.compare("") != 0)) {
		...
		setupDisplay(nodepath, ":", true, 0, row2, treeSelection);
...
Glib::RefPtr<Gtk::TreeSelection> EmapContainer::setupDisplay(
		std::string nodepath, std::string delimiter,
		bool usemodel, size_t pos, Gtk::TreeModel::Row row,
		Glib::RefPtr<Gtk::TreeSelection> treeSelection) {
	...
	if ((pos = nodepath.find(delimiter)) != std::string::npos) {
		...
		if (rownew) {
			...
			treeview->scroll_to_row(tpath);
			nodepath.erase(0, pos + delimiter.length());
		}
		Glib::RefPtr < Gtk::TreeStore > modelnull;
		return setupDisplay(nodepath, delimiter, false, pos, rownew,
				treeSelection);
	} else {
		...
		if (usemodel) {
			std::cout << "use model to get children" << std::endl;
			rownew = model->children()[stoi(nodepath)]; //The token row.
		} else {
			std::cout << "use row to get children" << std::endl;
			rownew = row->children()[stoi(nodepath)]; //The token row.
		}

		std::cout << "got rownew" << std::endl;
...
void EmapContainer::save_state(std::string rootdir, std::string path,
		std::string nodepath, std::string label, int bank, int program) {

	std::cout << "save state to json fie: with rootdir: " << rootdir
			<< std::endl;
	std::cout << "path: " << path << std::endl;
	std::cout << "nodepath: " << nodepath << std::endl;
	std::cout << "label: " << label << std::endl;
