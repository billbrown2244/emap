...
EmapContainer::EmapContainer(fluid_synth_t* synth_new, bool is_lv2) {
	...
	if (!is_lv2) {
		...
		container = (new Gtk::Grid())->gobj();		//Gtk::Table()
		scrolled = new Gtk::ScrolledWindow();
		path_container = new Gtk::Grid();		//1, 4); //2 elements
		...
		*/
		...
		(Glib::wrap(container))->attach(*path_container, 0, 0, 1, 1);
		...
		//finally add the container to the main window and show all children
				...
				std::cout << "making EMAP container (fluidsynth UI)"
						<< std::endl;

				GtkWindow* emap2 = GTK_WINDOW(
				...
				GtkWidget* path_container2 = gtk_grid_new();
				...
				GtkWidget* expand_all_button2 = gtk_button_new_with_label(
						"Expand All");
				...
				//setup the TreeView
				modelc = gtk_tree_store_new(4, G_TYPE_STRING, G_TYPE_STRING,
				...
				g_signal_connect(selection, "changed",
						G_CALLBACK(&EmapContainer::on_selection_changedLv2),
						this);
				...
				/* new */
				gtk_grid_attach(GTK_GRID(container2), set_root_folder_button2,
						0, 0, 1, 1);
				gtk_grid_attach(GTK_GRID(container2), expand_all_button2, 1, 0,
						1, 1);
				gtk_grid_attach(GTK_GRID(container2), collapse_all_button2, 2,
						0, 1, 1);
				//*/

				//Setup the ScrolledWindow and add the TreeView in it
				gtk_container_add((GtkContainer*) scrolled2,
						(GtkWidget*) treeview2);
				...
				/*new
				 gtk_grid_attach(GTK_GRID(container2), path_container2, 0, 0, 1, 1);
				...
				loadTreeLv2(root_folder.c_str(), root_folder.c_str(), row,
						modelc);
		...
		void EmapContainer::on_selection_changedLv2(GtkWidget *widget,
			...
			if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
				...
				gtk_tree_model_get(model, &iter, NAME, &name, PATH, &path, BANK,
						&bank, PROGRAM, &program, -1);
				...
				if (is_soundfont(path) || (bank != -1 && program != -1)) {
					std::cout << "name: " << name
							<< " is a soundfont lets open it." << std::endl;
					...
					if (bank != -1 && program != -1) {
						...
						fluid_sfont_t* soundfont = fluid_synth_get_sfont(synth,
								0);
						...
						std::cout << "pexistingpreset before: "
								<< existingpreset << std::endl;
						...
						if (HASH_COUNT(emap->m_cpresets) == 0
							...
							while (soundfont->iteration_next(soundfont, &preset)) {
								std::cout << "soundfont->iteration_next: "
										<< std::endl;
								...
								gtk_tree_store_append(emap->modelc, &child2,
										&iter);
							...
							cpresets* presetadd = (cpresets*) malloc(
									sizeof(cpresets));

							std::cout << "allocated preset: " << presetadd
									<< std::endl;
							...
							HASH_ADD_STR(emap->m_cpresets, soundfont_key,
									presetadd);
							...
							std::cout << "presets size after: "
									<< HASH_COUNT(emap->m_cpresets)
									<< std::endl;

						} else {
							std::cout
									<< "(presets.find(name) == presets.end()) false"
									<< std::endl;
						}
					...
					std::cout << "name: " << name
							<< " is just a folder.  do nothing. " << std::endl;
				}
		...
		void EmapContainer::on_button_clicked(EmapContainer* emap) {
			...
			std::cout << "root_folder before:  " << emap->root_folder
					<< std::endl;
			...
			case (Gtk::RESPONSE_OK): {
				...
				loadTree(emap->root_folder.c_str(), emap->root_folder.c_str(),
						row);
		...
		void EmapContainer::on_button_clickedLv2(EmapContainer* emap) {
			...
			dialog = gtk_file_chooser_dialog_new("Please choose a folder",
					GTK_WINDOW(emap->emap), action, "Cancel",
			...
			case (GTK_RESPONSE_ACCEPT): {
				...
				std::cout << "set new root folder: "
						<< emap->root_folder.c_str() << std::endl;

				GtkTreeIter* row;	//pass an empty row.

				loadTreeLv2(emap->root_folder.c_str(),
						emap->root_folder.c_str(), row, emap->modelc);
			...
			for (std::vector<int>::size_type i = 0; i != sorted.size(); i++) {
				...
				if (file_info->get_file_type() == Gio::FILE_TYPE_DIRECTORY) {
					...
					if (strcmp(orig_path, path) == 0) {
						...
						gtk_tree_store_set(model, &child, NAME,
								fileName.c_str(), PATH, result, BANK, -1,
								PROGRAM, -1, -1);
		...
		void EmapContainer::set_root_folder(const char* root_folder) {
			...
			for (int i = 0; i <= 1; i++) {
				const char* dir_path =
						(std::string(home_dir) + dirs[i]).c_str();
				mkdir(dir_path, 0755);
		...
		void EmapContainer::set_root_folderLv2(EmapContainer* emap) {
			...
			for (int i = 0; i <= 1; i++) {
				const char* dir_path =
						(std::string(emap->home_dir) + dirs[i]).c_str();
				mkdir(dir_path, 0755);
				//std::cout << "created directory path: " << dir_path << " " << mkdir_result
				//		<< std::endl;
...
//lv2 stuff
		static LV2UI_Handle instantiate(const _LV2UI_Descriptor * descriptor,
			...
			if (!emap->map) {
				std::cout << "EMAP error: Host does not support urid:map"
						<< std::endl;
				free(emap);
				return NULL;
			}
		...
		void EmapContainer::send_ui_state(EmapContainer* emap) {
			...
			//may need later
			LV2_Atom* obj = (LV2_Atom*) lv2_atom_forge_resource(&emap->forge,
					&frame, 0, emap->uris.ui_State);
			...
			//program
			lv2_atom_forge_property_head(&emap->forge, emap->uris.ui_program,
					0);
			...
			std::cout << "lv2_atom_total_size(msg): "
					<< lv2_atom_total_size(msg) << std::endl;
		...
		static void cleanup(LV2UI_Handle ui) {
			...
			std::cout << "emap name on cleanup: " << (emap->name == NULL)
					<< std::endl;
			std::cout << "emap path on cleanup: " << (emap->path == NULL)
					<< std::endl;
			std::cout << "emap bank on cleanup: " << (emap->bank == 0)
					<< std::endl;
			std::cout << "emap program on cleanup: " << emap->program
					<< std::endl;
		...
		static LV2UI_Descriptor descriptors[] = { { EMAP_UI_URI, instantiate,
				cleanup, port_event, NULL } };
